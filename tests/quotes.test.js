...
test('une ligne Excel ambiguë exige une clarification humaine révisée avant la prévisualisation', async () => {
  const document = readDb().quotes.find(value => value.title === 'Planning client contrôlé'), beforeReservations = readDb().reservations.length, csv = Buffer.from('Date;Prestation;Salle;Heure début;Heure fin\n20/06/2030;Prestation inconnue;Salle Montage A;09:00;18:00');
  const analyzed = await request(`/api/v1/quotes/${document.id}/client-planning/analyze`, { method: 'POST', body: JSON.stringify({ version: document.version, quoteVersionId: document.currentVersionId, fileName: 'planning-a-clarifier.csv', mimeType: 'text/csv', contentBase64: csv.toString('base64') }) }, admin);
  assert.equal(analyzed.response.status, 201, JSON.stringify(analyzed.data)); assert.equal(analyzed.data.summary.unmatched, 1); const sourceRow = analyzed.data.rows[0], item = { quoteLineId: document.lines[0].id, startDate: sourceRow.startDate, durationDays: 1, resourceIds: ['resource_3'], status: 'option', startTime: '09:00', endTime: '18:00', clientPlanningRowId: String(sourceRow.rowNumber) }, payload = { version: document.version, quoteVersionId: document.currentVersionId, clientPlanningImportId: analyzed.data.id, items: [item] };
  const blocked = await request(`/api/v1/quotes/${document.id}/planning-conversion/preview`, { method: 'POST', body: JSON.stringify(payload) }, admin); assert.equal(blocked.response.status, 409); assert.equal(blocked.data.error.code, 'CLIENT_PLANNING_CLARIFICATION_REQUIRED');
  const storedBefore = readDb().clientPlanningImports.find(value => value.id === analyzed.data.id), correction = { rowNumber: sourceRow.rowNumber, suggestedQuoteLineId: document.lines[0].id, suggestedResourceId: 'resource_3', startDate: sourceRow.startDate, durationDays: 1, startTime: '09:00', endTime: '18:00', resolutionNote: 'Salle et prestation confirmées par la production', confirmed: true }, revisionPayload = { importVersion: storedBefore.version, corrections: [correction] };
  const revised = await request(`/api/v1/quotes/${document.id}/client-planning/${analyzed.data.id}/revisions`, { method: 'POST', headers: { 'Idempotency-Key': 'clarify-client-planning-row' }, body: JSON.stringify(revisionPayload) }, admin); assert.equal(revised.response.status, 201, JSON.stringify(revised.data)); assert.equal(revised.data.currentRevisionNumber, 1); assert.equal(revised.data.rows[0].matchStatus, 'confirmed');
  const replay = await request(`/api/v1/quotes/${document.id}/client-planning/${analyzed.data.id}/revisions`, { method: 'POST', headers: { 'Idempotency-Key': 'clarify-client-planning-row' }, body: JSON.stringify(revisionPayload) }, admin); assert.equal(replay.response.status, 200); assert.equal(replay.data.currentRevisionNumber, 1);
  const storedAfter = readDb().clientPlanningImports.find(value => value.id === analyzed.data.id); assert.equal(storedAfter.analysis.rows[0].matchStatus, 'unmatched'); assert.equal(storedAfter.revisions.length, 1); assert.ok(readDb().auditEvents.some(value => value.action === 'quote.clientPlanningClarified' && value.entityId === analyzed.data.id));
  const preview = await request(`/api/v1/quotes/${document.id}/planning-conversion/preview`, { method: 'POST', body: JSON.stringify(payload) }, admin); assert.equal(preview.response.status, 200, JSON.stringify(preview.data)); assert.equal(readDb().reservations.length, beforeReservations);
});
