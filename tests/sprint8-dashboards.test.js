...
test('Vue d’ensemble câble l’API et une interface progressive accessible', () => {
  ...
  assert.match(app, /dashboardWithExactComparisonPeriods/); assert.match(app, /dashboardComparisonPeriodLabel\(selected\.from,selected\.to\)/); assert.match(app, /dashboardComparisonPeriodLabel\(current\.from,current\.to\)/); assert.match(app, /getUTCDate\(\)===1\?'1er'/); assert.match(css, /\.overview-comparison-periods strong/);
