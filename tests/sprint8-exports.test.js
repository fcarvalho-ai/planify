...
test('S8-B expose Planning XLSX/PDF et KPI XLSX avec bornes et permissions', async t => {
  ...
  assert.equal(kpi.status, 200); assert.equal(kpiBuffer.subarray(0, 2).toString(), 'PK'); assert.match(kpiBuffer.toString('utf8'), /invoicedRevenue/); assert.match(kpiBuffer.toString('utf8'), /Indisponible|unavailable/); for (const sheet of ['Synthèse', 'Détail', 'Sections', 'Définitions']) assert.match(kpiBuffer.toString('utf8'), new RegExp(`name="${sheet}"`)); assert.match(kpiBuffer.toString('utf8'), /forecast/);
  ...
  const inclusive367 = await fetch(`${base}/api/v1/exports/planning.xlsx?from=2025-01-01&to=2026-01-02`, { headers: { cookie: admin } }); assert.equal(inclusive367.status, 422); assert.equal((await inclusive367.json()).error.code, 'EXPORT_PERIOD_INVALID');
