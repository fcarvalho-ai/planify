...
test('confirmation Actual idempotente, versionnée et auditable sans seconde écriture', async () => {
  ...
  assert.equal(confirmed.response.status, 201, JSON.stringify(confirmed.data)); assert.equal(confirmed.data.version, 1); assert.equal(confirmed.data.currentRevision.quantityMilli, confirmed.data.plannedSnapshot.quantityMilli); assert.equal(confirmed.data.currentRevision.confirmationKind, 'confirmed'); assert.equal(confirmed.data.currentRevision.priorRevisionId, null); assert.equal(confirmed.data.currentRevision.digestVersion, 3); assert.ok(['resolved', 'unavailable'].includes(confirmed.data.currentRevision.costSnapshot.state)); assert.match(confirmed.data.currentRevision.sourceDigest, /^[a-f0-9]{64}$/); assert.equal(confirmed.data.reconciliation.state, 'compliant'); assert.equal(confirmed.data.reconciliation.commercialState, 'unmatched'); assert.equal(confirmed.data.reconciliation.billableQuantityMilli, '0');
