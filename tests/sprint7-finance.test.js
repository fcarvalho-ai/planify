...
let server; let baseUrl; let admin; let viewer; let financeParis;
...
before(async () => {
  resetData(makeSeed());
  const seeded = readDb(), template = seeded.users.find(value => value.id === 'user_viewer'), timestamp = new Date().toISOString(), userId = 'user_finance_paris', membershipId = 'membership_finance_paris', roleId = 'role_finance_manager', boulogneUserId = 'user_finance_boulogne_scope', boulogneMembershipId = 'membership_finance_boulogne_scope';
  seeded.users.push({ ...template, id: userId, email: 'finance.paris@northlight.fr', displayName: 'Finance Paris', role: 'financeManager', siteIds: ['site_paris'] }, { ...template, id: boulogneUserId, email: 'finance.boulogne.scope@northlight.fr', displayName: 'Personne Boulogne', role: 'viewer', siteIds: ['site_boulogne'] });
  seeded.organizationMemberships.push({ id: membershipId, userId, companyId: 'company_northlight', displayName: 'Finance Paris', status: 'active', defaultSiteId: 'site_paris', version: 1, createdAt: timestamp, updatedAt: timestamp }, { id: boulogneMembershipId, userId: boulogneUserId, companyId: 'company_northlight', displayName: 'Personne Boulogne', status: 'active', defaultSiteId: 'site_boulogne', version: 1, createdAt: timestamp, updatedAt: timestamp });
  seeded.roles.push({ id: roleId, companyId: 'company_northlight', code: 'financeManager', name: 'Finance gestion', permissions: ['finance.read', 'finance.cost.manage'], systemManaged: false, active: true, version: 1, createdAt: timestamp, updatedAt: timestamp });
  seeded.membershipRoles.push({ membershipId, roleId });
  seeded.membershipScopes.push({ membershipId, scope: 'sites', siteIds: ['site_paris'], organizationUnitIds: [], entityScopes: { client: ['client_1'] } }, { membershipId: boulogneMembershipId, scope: 'sites', siteIds: ['site_boulogne'], organizationUnitIds: [] });
  ...
  server = createServer(); await new Promise((resolve, reject) => { server.once('error', reject); server.listen(0, '127.0.0.1', resolve); }); baseUrl = `http://127.0.0.1:${server.address().port}`; admin = await login('admin@northlight.fr'); viewer = await login('viewer@northlight.fr'); financeParis = await login('finance.paris@northlight.fr');
});
