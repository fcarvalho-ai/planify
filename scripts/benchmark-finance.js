...
function main() {
  ...
  const runMargins = () => financeMargins(db, auth, { projectId: project.id, asOf: '2026-08-23' }), runBacklog = () => financeBacklog(db, auth, { projectId: project.id, asOf: '2026-08-23' }), runForecast = () => financeForecast(db, auth, { projectId: project.id, asOf: '2026-08-23' }), runOccupancy = () => financeOccupancy(db, auth, { from: '2026-08-01', to: '2026-08-01', groupBy: 'day', dimension: 'resource' }), runAnnualOccupancy = () => financeOccupancy(db, auth, { from: '2026-01-01', to: '2026-12-31', groupBy: 'month', dimension: 'site' }), runProfitability = () => financeProfitability(db, auth, { projectId: project.id, asOf: '2026-08-23', dimension: 'resourceId' }), runUnbilled = () => financeUnbilledOverages(db, auth, { projectId: project.id, asOf: '2026-08-23' }), runDiscounts = () => financeRateDiscounts(db, auth, {}); runMargins(); runBacklog(); runForecast(); runOccupancy(); runAnnualOccupancy(); runProfitability(); runUnbilled(); runDiscounts();
  const margins = measure(8, runMargins), backlog = measure(8, runBacklog), forecast = measure(8, runForecast), occupancy = measure(8, runOccupancy), annualOccupancy = measure(5, runAnnualOccupancy), profitability = measure(8, runProfitability), unbilled = measure(8, runUnbilled), discounts = measure(8, runDiscounts), sample = runMargins(), output = { dataset: { resources: resources.length, reservations: 10000, commercialDocuments: db.quotes.length, actualRecords: db.actualRecords.length, projectCosts: db.projectCosts.length, clientId: client.id }, latencyMs: { margins, backlog, forecast, occupancy, annualOccupancy, profitability, unbilled, discounts }, result: { itemCount: sample.itemCount, signedRevenueMinor: sample.totals.signedRevenueMinor, plannedCostMinor: sample.totals.plannedCostMinor, actualCostMinor: sample.totals.actualCostMinor }, thresholdMs: { readP95: 300 } };
  console.log(JSON.stringify(output, null, 2)); if ([margins, backlog, forecast, occupancy, annualOccupancy, profitability, unbilled, discounts].some(value => value.p95 >= 300)) process.exitCode = 1;
}
